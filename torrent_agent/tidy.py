...
# classified and left where they are. "Specials" is deliberately absent: that
# is season 0, real episodes TVmaze can name.
_EXTRAS_DIRS = {
...
def _extras_note(source: Path) -> str | None:
    """A line saying extras were left in place, or None if there are none."""
    if source.is_file():
        return None
    extras = [
        p for p in source.rglob("*")
        if p.is_file() and p.suffix.lower() in MEDIA_SUFFIXES and _is_extra(source, p)
    ]
    if not extras:
        return None
    return f"{len(extras)} extra(s) left in place, not filed"
